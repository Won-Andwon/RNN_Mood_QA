        ...
        with tf.device("/cpu:0"), word2vec_graph.name_scope("Word2Vec"):
            with tf.Session() as sess:
                model = Word2VecModel(session=sess,
                                      save_path='/home/chenyongchang/data/movie_subtitles_en/w2v',
                                      embedding_size=512,
                                      dictionary=word2id,
                ...
                print(model.get_ids_embeddings('here'))
                ...
                with tf.Session(graph=word2vec_graph) as word2vec_session:
                    ...
                    for i in xrange(num_steps):
                        ...
                        if i % (parameters.word2vec_checkpoint_interval * 10) == 0:
                            ...
                            step_time = now / (parameters.word2vec_checkpoint_interval * 10)
                            start_time = time.time()
                            if i > 0:
                                average_loss /= (parameters.word2vec_checkpoint_interval * 10)
                            # The average loss is an estimate of the loss over the last 2000 batches.
                            print("[Word to Vector Model] Average loss at step ", i, ": ", average_loss)
                            print("[Word to Vector Model] Average time at step ", i, ": %0.6f" % step_time)
...
if __name__ == '__main__':
    word2vec_test(r'/home/chenyongchang/data/movie_subtitles_en/w2v/word2vec_model.ckpt',
                  [
                      'where',
                      'been',
                      'right',
    ...
    file_reader = open(
        r"/home/chenyongchang/data/movie_subtitles_en/vocabulary_vector40000.txt", mode='r', encoding="UTF-8")
    no = 82
    linescounter = 0
