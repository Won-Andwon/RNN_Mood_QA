...
class CNNModel(object):
    ...
    def build_graph(self):
        ...
        self.full_connect_gen = tf.matmul(res_flat, self.fcw, transpose_b=True) + self.fcb
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        vocabulary_size = len(id2word)
        cnn_model = CNNModel(sess,
                             saved_model=r"D:\Data\dia\cnn\cnn_model.ckpt-0")
        ...
        fenci = data_utils.basic_tokenizer("我的快乐就是学到知识与经验 学会做事做人")
        print(fenci)
...
#         #     tf.nn.sigmoid_cross_entropy_with_logits(logits=connect_out, labels=self.batch_labels),
#         #     name="cnn_loss")
#         self.pos_train_op = tf.train.AdamOptimizer(1e-3).minimize(self.pos_loss, global_step=self.global_step)
